...
Automatically matches unmatched resumes to relevant jobs using the backend's FAISS vector search and re-ranking pipeline.
...
3. Calls the backend's `getMatches(rawText, parsedData)` service which:
   - FAISS vector similarity search (top 20 candidates)
