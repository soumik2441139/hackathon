        ...
        <ProtectedRoute requiredRole="student">
            <main className="pt-32 pb-24 px-6 min-h-screen overflow-x-hidden bg-[var(--background)] selection:bg-orange-500/30">
                <div className="max-w-6xl mx-auto">
                    ...
                    >
                        {/* Hero Section */}
                        <motion.div variants={itemVariants} className="relative rounded-[3rem] overflow-hidden bg-gradient-to-br from-[#1E1E24] to-[#121214] border border-white/5 shadow-2xl p-10 md:p-14">
                            {/* Ambient Glows */}
                            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-orange-500/10 blur-[100px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/2" />
                            <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-yellow-500/15 blur-[100px] rounded-full pointer-events-none translate-y-1/2 -translate-x-1/4" />

                            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-12">
                                <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
                                    <div
                                        className="relative w-32 h-32 rounded-[2.5rem] bg-gradient-to-br from-yellow-500 via-orange-500 to-yellow-500 p-[2px] cursor-pointer group shadow-[0_0_40px_rgba(249,115,22,0.2)] hover:shadow-[0_0_60px_rgba(249,115,22,0.4)] transition-all duration-500"
                                        onClick={() => fileInputRef.current?.click()}
                                    >
                                        <div className="w-full h-full rounded-[2.4rem] bg-[#18181B] flex items-center justify-center text-4xl font-black text-white overflow-hidden relative">
                                            ...
                                            <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm">
                                                {isUploading ? <Loader2 className="animate-spin text-orange-400 mb-1" size={24} /> : <Camera className="text-orange-400 mb-1" size={24} />}
                                                <span className="text-[10px] font-bold uppercase tracking-widest text-orange-400">Update</span>
                                            </div>
                                        </div>
                                        <input
                                            type="file"
                                    ...
                                    <div>
                                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/20 text-[10px] font-black uppercase tracking-[0.2em] text-orange-400 mb-4 backdrop-blur-md">
                            ...
                            {[
                                ...
                                <div key={i} className={`glass-card p-8 bg-gradient-to-br ${s.color} border border-white/5 hover:${s.border} transition-all duration-300 group overflow-hidden relative`}>
                                    ...
                                    <div className="flex justify-between items-start mb-4 relative z-10">
                                        <div className={`w-14 h-14 rounded-2xl bg-[#18181B] border border-white/5 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform ${s.iconColor}`}>
                                            {s.icon}
                                        </div>
                                    </div>
                        ...
                        <motion.div variants={itemVariants} className="glass-card rounded-[2.5rem] p-4 md:p-8 border-white/5 bg-[#18181B]/80">
                            <div className="flex items-center gap-2 mb-8 p-1 bg-white/5 border border-white/10 rounded-2xl w-fit">
                                <button
                                    ...
                                    className={`px-8 py-3 rounded-xl text-sm font-bold tracking-wide transition-all ${activeTab === 'applications' ? 'bg-orange-500 text-brand-dark shadow-[0_0_20px_rgba(249,115,22,0.3)]' : 'text-white/50 hover:text-white'}`}
                                >
                                    My Applications
                                </button>
                                <button
                                    onClick={() => setActiveTab('saved')}
                                    className={`px-8 py-3 rounded-xl text-sm font-bold tracking-wide transition-all ${activeTab === 'saved' ? 'bg-yellow-500 text-white shadow-[0_0_20px_rgba(234,179,8,0.3)]' : 'text-white/50 hover:text-white'}`}
                                >
                                    Saved Opportunities
                                </button>
                            </div>
                        ...
                        {/* Explore Banner */}
                        <motion.div variants={itemVariants} className="relative rounded-[2.5rem] overflow-hidden bg-orange-500 text-brand-dark p-10 md:p-14 border border-orange-500 shadow-[0_0_40px_rgba(249,115,22,0.1)] group">
